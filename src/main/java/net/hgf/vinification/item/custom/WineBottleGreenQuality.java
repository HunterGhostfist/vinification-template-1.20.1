...
public class WineBottleGreenQuality extends HoneyBottleItem {
    private static final int MAX_USE_TIME = 40;


    public WineBottleGreenQuality(Settings settings) {
        super(settings);
    }
    ...
    public ItemStack finishUsing(ItemStack stack, World world, LivingEntity user) {
        ...
        if (!world.isClient) {
            user.addStatusEffect(new StatusEffectInstance(StatusEffects.NIGHT_VISION, 15*20*60, 1));
            user.addStatusEffect(new StatusEffectInstance(ModEffects.STICKY, 15*20*60, 0));
        }
        return stack;
    }
