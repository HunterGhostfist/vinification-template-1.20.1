...
public class WineBottlePurpleQuality extends HoneyBottleItem {
    private static final int MAX_USE_TIME = 40;


    public WineBottlePurpleQuality(Settings settings) {
        super(settings);
    }
    ...
    public ItemStack finishUsing(ItemStack stack, World world, LivingEntity user) {
        ...
        if (!world.isClient) {
            user.addStatusEffect(new StatusEffectInstance(StatusEffects.SLOW_FALLING, 15*20*60, 1));
            user.addStatusEffect(new StatusEffectInstance(ModEffects.VOID_CRAWLER, 15*20*60, 0));
        }
        return stack;
    }
