...
public class WineBottleRedQuality extends HoneyBottleItem {
    private static final int MAX_USE_TIME = 40;


    public WineBottleRedQuality(Settings settings) {
        super(settings);
    }
    ...
    public ItemStack finishUsing(ItemStack stack, World world, LivingEntity user) {
        ...
        if (!world.isClient) {
            user.addStatusEffect(new StatusEffectInstance(StatusEffects.SPEED, 15*20*60, 1));
            user.addStatusEffect(new StatusEffectInstance(ModEffects.INCINERATE, 15*20*60, 0));
        }
        return stack;
    }
